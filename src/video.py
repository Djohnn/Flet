...
def video_page(page: Page):
    page.title = "Vídeo"
    page.vertical_alignment = MainAxisAlignment.CENTER
    page.horizontal_alignment = CrossAxisAlignment.CENTER
    page.scroll = "auto"

    if not page.session.contains_key("video_links"):
        page.session.set("video_links", [])
    ...
    def update_content():
        ...
        for index, item in enumerate(page.session.get("video_links")):
            content_scroll.content.controls.append(
                Container(
                    content=Column([
                        Icon(Icons.VIDEO_FILE, size=40, color=Colors.WHITE),
    ...
    def open_edit_modal(index):
        nonlocal edit_index
        edit_index = index
        item = page.session.get("video_links")[index]
    ...
    fundo_img = Stack([
        ...
        Column([
            Text("Vídeos de Treino", size=24, weight=FontWeight.BOLD, color=Colors.WHITE),
            Row([
    ...
    page.views.append(View("/video", [fundo_img]))
    update_content()
...
#     def update_content():
#         content_scroll.content.controls.clear()
#         for item in page.session.get("video_links"):
#             content_scroll.content.controls.append(
#                 Container(
#                     content=Column([
#                         Icon(Icons.VIDEO_FILE, size=40, color=Colors.WHITE),
